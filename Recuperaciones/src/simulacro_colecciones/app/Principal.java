package simulacro_colecciones.app;


import simulacro_colecciones.model.Empleado;
import simulacro_colecciones.model.Nomina;
import simulacro_colecciones.model.TDepartamento;
...
public class Principal {
    ...
    private void buscarEmpleado(String dni) {
        empleados.stream().filter(empleado -> empleado.getDni().equalsIgnoreCase(dni))
                .findFirst().ifPresentOrElse(System.out::println, () -> System.out.println("No se ha encontrado un empleado con ese DNI"));
    }
    ...
    private void mostrarNumeroEmpleadosPorDepartamento() {
        ...
        resultado.forEach((tDepartamento, cantidad) -> System.out.println(tDepartamento + " : " + cantidad));
    }

    private double porcentajeRetencionTrabajadoresNuevos() {
        YearMonth fechaHaceUnYear = YearMonth.now().minusMonths(12);
        return empleados.stream().filter(empleado -> empleado.fechaPrimeraNomina().isAfter(fechaHaceUnYear))
                .mapToDouble(empleado -> empleado.getUltimaNomina().getPorcentajeRetencion())
                .average().orElse(0.0);
    }

    private void empleadosQueMasCobran() {
        double salarioMax = empleados.stream().mapToDouble(empleado -> empleado.getUltimaNomina().getSalarioNeto()).max().orElse(0.0);

        empleados.stream().filter(empleado -> empleado.getUltimaNomina().getSalarioNeto() == salarioMax).forEach(System.out::println);
    }

    private void costeTotalSalarios(YearMonth fecha) {
        double totalSalario = empleados.stream().flatMap(empleado -> empleado.nominas.stream())
    ...
    private void mostrarTodosEmpleadosOrdenados() {
        //Ordenamos cada empleado por la fecha de la primera Nomina
        empleados.stream().sorted(Comparator.comparing(Empleado::fechaPrimeraNomina)
                //Luego ordenamos con thenComparing si hay empate por orden alfabetico de los apellidos
                .thenComparing(Empleado::getApellido)).forEach(System.out::println);
    }
